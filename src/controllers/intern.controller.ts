...
import { InternshipValidation, GetInternshipsQuerySchema, InternshipIdParamSchema } from '../validations/internship.validation';
...
export const postInternship = async (req: Request, res: Response) => {
  try {
    ...
    const newInternship = new InternshipModel({ ...internshipData, createdBy: userFromDb._id });
...
export const deleteInternship = async (req: Request, res: Response) => {
  try {
    ...
    if (internship.createdBy.toString() !== res.locals.user._id) {
      return res.status(401).json({ success: false, message: 'No permission to delete this internship' });
    }
...
export const updateInternship = async (req: Request, res: Response) => {
  try {
    ...
    if (internship.createdBy.toString() !== res.locals.user._id) {
      return res.status(401).json({ success: false, message: 'No permission to update this internship' });
    }

    await InternshipModel.findByIdAndUpdate(params.id, internshipData);
